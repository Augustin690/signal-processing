...
a=np.asarray([1,2,3])
b=np.asarray([1,1,0])
c=np.asarray([  ??? ]) #COMPLETER convolution des deux a calculer a la main (convolution circulaire taille 3)
...
321*121
N=max(len(h),len(g))
hz=np.concatenate((h,  ???)) #COMPLETER
gz=np.concatenate((g, ??? )) #COMPLETER
...
#%%
Fe=1000 ;Te=1/Fe ; #echantillonnage a 1000 Hz 
t=np.arange(0,300)*Te# t represente le temps en secondes de 300 echantillons

xi=115; # frequence de l'onde 
...
def une_colonne_spectrogramme(u,M,N,n):
    ...
    m=np.arange(n,???).astype(np.int)#COMPLETER (jusqu'au ou?)
