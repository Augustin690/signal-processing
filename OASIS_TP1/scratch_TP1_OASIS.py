...
np.asarray([1,2,3]).reshape((1,-1))
np.asarray([[1,2],[3,4]])
np.asarray([1,2,3,4]).reshape(2,2)
np.asarray([1,2,3,4]).reshape(-1,2)
...
def valconv(h,u): # cette version ne fonctionne pas telle quelle!
    ...
    for n in range(a+b-1):
        idx= np.arange(max(0,n+1-b),min(a-1,n)+1) # Formule trouvée précédemment: les indices pour h
        out[n]=(h[idx]*u[n-idx]).sum()
        
    return out

valconv(np.ones(3),np.ones(2))
r = np.arange(10)
valconv(np.cos(r),np.sin(r))

#%% ATTENTION de bien mettre 
repertoire='/cal/exterieurs/acurinier-21/OASIS_TP1/' #mettre le chemin ou vous avez mis les donnees
